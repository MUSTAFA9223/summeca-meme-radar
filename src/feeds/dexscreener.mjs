...
export async function fetchDexScreenerSnapshot(base, { timeoutMs = 5000 } = {}) {
  ...
  try {
    ...
    // DexScreener exposes rolling m5 data. For a brand-new pair, m5 is only as old
    // as the pair itself, so normalize the observed window to an approximate 30s rate.
