...
const PERFORMANCE_KEY = 'wallet_performance_v1';
...
function walletFromSignal(signal) {
  const wallets = Array.isArray(signal?.reason?.wallets) ? signal.reason.wallets : [];
  return wallets.map((w) => ({ address: String(w?.address || '').toLowerCase(), label: String(w?.label || ''), paidUsd: finite(w?.paid_usd) })).filter((w) => w.address);
}

function summarize(signals, tokenById) {
  const map = new Map();
  for (const signal of signals) {
    ...
    for (const w of walletFromSignal(signal)) {
      const row = map.get(w.address) || { address: w.address, label: w.label, samples: 0, peakRoiSum: 0, hit25: 0, hit50: 0, hit100: 0, paidUsd: 0, entryScoreSum: 0, riskSum: 0 };
      row.label ||= w.label;
      row.samples += 1;
...
export class WalletPerformanceWorker {
  ...
  async cycle() {
    ...
    try {
      ...
      const performance = summarize(Array.isArray(signals) ? signals : [], tokenById);
      if (this.settings.enabled) await this.settings.set(PERFORMANCE_KEY, JSON.stringify({ updatedAt: new Date().toISOString(), wallets: performance.slice(0, 50) })).catch(() => {});
